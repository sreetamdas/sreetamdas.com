...
For the first part of the puzzle, we need to perform 1000 "button presses". For
us, this will mean processing 1000 `:low` pulses sent from the `broadcast` module
and tracking the count of `:low` and `:high` pulses.
...
|> Enum.map(&{&1, :low, "broadcast"})
|> then(fn init_pulses ->
  # enumerator 1 to 1000
  1..1000
  # {parsed_input, {low_count, high_count}} = init_values
  |> Enum.reduce({parsed, {0, 0}}, fn
    # current element, accumulator
    _press_index, {map.{low_counts, high_counts}} ->
      # process pulse
      nil
  end)
end)
...
defmodule PulsePropagation do
  ...
  defp process_module({node, pulse, sender}, node_map) do
    node_map
  ...
  defp is_high_pulse_to_dependents(
         {_node, pulse, sender},
         dependents
       ) do
    cond do
      Enum.member?(dependents, sender) and pulse == :high ->
        true
  ...
  defp process_pulse([], node_map, _, counts),
  ...
  defp process_pulse(
    ...
    |> then(fn {next_modules, updated_map} ->
      ...
      |> then(fn {result, dependent_high} ->
        {result,
         dependent_high ||
           is_high_pulse_to_dependents(pulse, dependents)}
      end)
    end)
  end
