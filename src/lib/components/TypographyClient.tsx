...
export const ChameleonHighlight = ({
	...
	return (
		<span
			className={cn(
				"font-serif text-[clamp(4rem,10vw,8rem)] leading-none font-bold text-(--color-fancy-pants,var(--color-primary)) transition-colors duration-3000 ease-linear",
				passedClasses,
			)}
		>
			{children}
...
const Sparkle = ({ size, color, style }: Pick<Sparkle, "size" | "color" | "style">) => (
	<span className="absolute block motion-safe:animate-[grow-shrink_700ms_forwards]" style={style}>
		<svg
			width={size}
			height={size}
			viewBox="0 0 68 68"
