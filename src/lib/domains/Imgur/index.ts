...
type KeebDetailsFromNotion = Omit<KeebDetails, "image"> & {
...
export class ImgurClient {
  ...
  async getImgurAlbumImages() {
    const request = await fetch(
      `${this.base_url}/album/${this.album_url}/images`,
      {
        method: "GET",
        headers: {
    ...
    const response = (await request.json()) as ImgurAPIResponse<
      Array<ImgurImage>
    >;
  ...
  async addImgurImagesData(
    ...
    const enrichedImagesData = imagesData.map((imageData) => {
      ...
      const imgurImage = albumImagesData?.find(
        ({ link }) => link === image.url,
      );
