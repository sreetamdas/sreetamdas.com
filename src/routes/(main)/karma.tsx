  ...
  .handler(async () => {
    return await Promise.all(
      theme_language_map.map(async ({ name, default_image, light_image }) => {
        const default_dimensions = await imageSizeFromFile(
          path.join("./public", default_image.replace(/^\//, "")),
        );
        const light_dimensions = await imageSizeFromFile(
          path.join("./public", light_image.replace(/^\//, "")),
        );
