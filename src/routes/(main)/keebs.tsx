...
async function KeebsPage() {
  ...
  return (
    <>
      ...
      {keebs.length === 0 ? (
        <p className="pb-8">
          Keyboard data is temporarily unavailable in this preview deployment.
        </p>
      ) : null}
        ...
        {keebs.map(({ name, tags, image }) => (
          ...
          >
            <div className="grid grid-flow-col items-center justify-between gap-8">
              <h3 className="pt-0 font-serif text-3xl font-medium tracking-tight">
                {name}
              </h3>
...
async function getKeebsFromNotion(): Promise<
  Array<KeebDetails | KeebDetailsFromNotion>
> {
  if (
  ...
  let results: Awaited<
    ReturnType<typeof notionClient.queryDatabase>
  >["results"] = [];
  try {
  ...
  const keebsDetailsFormatted = results?.map((keebDetails) => {
    const keebDetailsFormatted = Object.keys(keebDetails.properties).reduce(
      (details, property) => {
